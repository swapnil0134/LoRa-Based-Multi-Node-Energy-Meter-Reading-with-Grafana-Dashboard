...
# Current: Code/Gateway/Sub/Dev1/Code/Device1.py
current_dir = os.path.dirname(os.path.abspath(__file__))
# Root: Go up 5 levels
root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))))
log_base_dir = os.path.join(root_dir, 'Energy_Meters/Sub/Dev1/Logs')

# InfluxDB configuration
influxdb_url = 'http://localhost:8086'
influxdb_token = 'token'
influxdb_org = 'smdh'
influxdb_bucket = 'Energy_Meter_Device_1'
...
def write_to_influxdb(timestamp, message):
    ...
    point = Point("sensor_data").tag("device", "Device1").time(timestamp, WritePrecision.S)
...
def receive_message():
    ...
    if packet is not None:
        ...
        # Check if the message contains the keyword "Device1"
        if "Device1" in packet_text:
            ...
            print("Received packet does not contain 'Device1'. Ignoring.")
