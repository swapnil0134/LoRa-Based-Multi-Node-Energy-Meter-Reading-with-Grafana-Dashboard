...
# Current: Code/Gateway/Sub/Dev3/Code/Device3.py
current_dir = os.path.dirname(os.path.abspath(__file__))
# Root: Go up 5 levels
root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))))
log_base_dir = os.path.join(root_dir, 'Energy_Meters/Sub/Dev3/Logs')

# InfluxDB configuration
influxdb_url = 'http://localhost:8086'
influxdb_token = 'token'
influxdb_org = 'smdh'
influxdb_bucket = 'Energy_Meter_Device_3'
...
def write_to_influxdb(timestamp, message):
    ...
    point = Point("sensor_data").tag("device", "Device3").time(timestamp, WritePrecision.S)
...
def receive_message():
    ...
    if packet is not None:
        ...
        # Check if the message contains the keyword "Device3"
        if "Device3" in packet_text:
            ...
            print("Received packet does not contain 'Device3'. Ignoring.")
